...
with tempfile.TemporaryDirectory() as tmp:
    ...
    if format_choice == "JSON":
        ...
        st.markdown(f"#### {path.name}  ({len(text):,} bytes)")
    ...
    st.markdown("#### Round-trip check")
    stats_row(
        [
            ("wildtype match", str(restored.wildtype == sim.wildtype)),
            ("n match", str(restored.n == sim.n)),
            ("phenotypes allclose", str(bool(np.allclose(restored.phenotypes, sim.phenotypes)))),
        ]
    )
